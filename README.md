...
flowchart LR
    ...
    VK2 -- "*SET:KEY" --> PK2
    VC2 -- "*SET:DISP ..." --> PD2
    NOTE["★ 1Hz 全量心跳自动校准，丢失事件自动恢复，无需重传"]
```
...
│   │   ├── systick.c / systick.h           # SysTick 系统时基
...
│   ├── Objects/                            # 编译输出目录
│   │   └── exp1.axf                        # ★ 必交的可烧写固件文件
│   ├── Listings/                           # 链接 map / 汇编列表文件
│   ├── RTE/                                # Keil Run-Time Environment
│   │   ├── Device/TM4C1294NCPDT/           # 启动文件（startup_*.s）
│   │   └── _Target_1/                      # RTE 配置
│   ├── hw.uvprojx                          # Keil 工程文件
│   └── hw.uvoptx                           # Keil 工程选项文件
├── pc_host/                                # PC 上位机工程（Python 3.11 + PyQt5）
│   ├── ui/                                 # Qt Designer UI 文件
│   │   ├── main_window.ui                  # 主窗口布局（Qt Designer 源文件）
│   │   └── ui_main_window.py               # .ui 编译生成的 Python 界面类
│   ├── main.py                             # ★ 程序入口与主界面协调
│   ├── serial_worker.py                    # 后台串口收发线程（QThread）
│   ├── protocol.py                         # 协议解析与帧处理模块
│   ├── twin_panel.py                       # 数字孪生镜像面板（自绘 QWidget）
│   ├── ntp_helper.py                       # NTP 网络对时辅助模块           [E1]
│   ├── weather_helper.py                   # wttr.in 天气获取与映射模块     [E2]
│   ├── astral_helper.py                    # Astral 日出日落计算模块        [E3]
│   ├── chart_widget.py                     # Matplotlib 数据可视化图表组件  [E4]
│   ├── log_store.py                        # 事件日志持久化存储（events.csv）
│   ├── requirements.txt                    # Python 依赖清单（含完整传递依赖）
│   └── run_host.bat                        # Windows 一键启动脚本
├── docs/                                   # 文档目录
│   ├── 大作业524442910013-江彦佐.pdf        # ★ 简介报告（4-8 页）
│   └── 演示视频.mp4                         # ★ ≤5 分钟带旁白演示视频                                 
└── README.md                               # ★ 本说明文档
```

> **标注 ★ 者为提交必选项**，缺一将按评分标准扣分（每项 -10）。
...
```mermaid
flowchart TB
    subgraph MCU["<b>S800 板端 (MCU)</b>"]
        K1["I²C 按键 (K1-K8)"] --> CTRL["<b>主控制器<br/>TM4C1294</b>"]
        K2["GPIO USER1/USER2"] --> CTRL
        CTRL --> SEG["8位 7SEG 数码管<br/>(I²C TCA6424)"]
        CTRL --> LED["8位 LED<br/>(PCA9557)"]
        CTRL --> BEEP["蜂鸣器 PK5<br/>(闹钟/反馈)"]
    end

    subgraph PC["<b>PC 上位机 (PyQt5)</b>"]
        CP["控制面板<br/>(12条命令)"] --> SW["串口后台线程<br/>(SerialWorker)"]
        TP["数字孪生镜像面板"] --> PP["协议解析模块<br/>(protocol.py)"]
        LG["收发日志<br/>(四色编码)"] --> PP
        CH["数据看板"] --> PP
        SW --> PP
        EX["扩展模块<br/>NTP/天气/昼夜/图表"] --> NET["🌐 互联网"]
    end

    MCU <==>"UART0 虚拟串口<br/>115200 8N1 无流控"==> PC
    PC -->|"  *SET:KEY 下行回路"| MCU
    MCU -->|"  *EVT 事件上行"| PC
```
...
> 📦 **完整冻结版本**（含所有传递依赖）见 `pc_host/requirements.txt`。
...
> 💡 **备选烧录**：也可使用 [LM Flash Programmer](https://www.ti.com/tool/LMFLASHPROGRAMMER) 直接烧录 `.axf` 或 `.bin` 文件。
...
| **1** | LED2 | **ALM** | 闹钟使能 | 常亮 = 闹钟已设防；快闪 = 正在响铃 |
| **2** | LED3 | **RING** | 闹钟响铃 | 亮 = 正在响铃（与 LED2 快闪同步） |
| **3** | LED4 | **EDIT** | 编辑模式 | 亮 = 当前处于编辑状态 |
| **4** | LED5 | **RX/TX** | 串口收发活动 | 串口收发后亮约 100ms 熄灭 |
| **5** | LED6 | **SUN** | 晴天指示 [E2] | 亮 = 天气为晴（Sunny/Clear） |
| **6** | LED7 | **RAI/SNO** | 雨雪指示 [E2] | 慢闪 = 雨/雪天气 |
| **7** | LED8 | **HOT** | 高温指示 [E2] | 亮 = 温度 ≥ 30°C |
| **7** | LED8 | **NTP** | NTP 对时状态 [E1] | 常亮 = 已同步；慢闪 = 超过 24 小时未同步 |

> **必做四类**：心跳（LED1）、闹钟状态（LED2+LED3）、编辑模式（LED4）、串口收发活动（LED5）  
> **扩展指示**：天气编码（LED6+LED7+LED8，或 NTP 状态）
...
| # | 测试项 | 操作 | 预期结果 |
|:--:|--------|------|----------|
| 1 | 开机画面 | 上电/复位 | `88888888` → 灭 → 学号闪烁 → 姓名闪烁 → `V101` → 走时 |
