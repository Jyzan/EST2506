...
| 功能 | 关键要求 |
|------|----------|
| **情景倒计时系统** | MCU 端五态有限状态机（IDLE→EDIT→RUN⇄PAUSE→DONE），板上 EXT 键独立编辑与运行，SHIFT 分/秒切换，ADD/SAVE 加减。三种到点情景：滚动文本、闪烁庆祝、静默。PC 端 CountdownRing 圆弧进度环 + TTS 语音倒数 ≤5 秒联动。8 LED 按剩余比例实时进度条。协议含 TIME/SCENE/MSG/START/PAUSE/RESUME/STOP 七子命令族，*EVT:CD STATE 1Hz 心跳 + *EVT:CD DONE 到点事件。 |
...
| **EXT** | K8 | SW8 | 倒计时：闲置态进入编辑 / 编辑态确认启动 / 运行态暂停 / 暂停态继续 / 到点态退出。非倒计时态上报 `*EVT:KEY EXT` | 倒计时运行中：**停止/取消倒计时** |

### 8.2 GPIO 独立按键

| 标签 | MCU 引脚 | 电平 | 功能 |
|------|----------|------|------|
| **USER1** | PJ0 | 低电平按下 | 短按上报 `*EVT:KEY USER1`(PC 自动触发 NTP 对时)；长按同样上报 `*EVT:KEY USER1` + 板端短显 NTP 同步状态 |
| **USER2** | PJ1 | 低电平按下 | 短按上报 `*EVT:KEY USER2`(数码管短显天气 5 秒)；长按同上报 `*EVT:KEY USER2` |
...
| **定时检查** | PC 每分钟检查一次，自动模式用 PC 本地时间判定；手动「应用昼夜」按钮先 `*GET:TIME` 获取 MCU 钟表时间再判定 |
...
| 图表 | 类型 | 数据内容 | 说明 |
|------|------|----------|------|
| **Alarm by Hour** | 折线图 | 闹钟触发的小时分布 | 统计各时段闹钟触发频率 |
| **NTP Delta (ms)** | 柱状图 | NTP 对时误差时间序列 | 反映本地时钟漂移趋势 |
| **Key Heat** | 条形图 | 各类按键使用频次 | 统计 10 种按键热度 |
