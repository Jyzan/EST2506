...
```
┌─────────────────────────────────────────────────────┐
│  S800 板端 (物理世界)          PC 上位机(数字世界)    │
│                                                     │
│  按键按下 ──── *EVT:KEY ────▶  虚拟按键高亮 + 日志   │
│  显示变化 ──── *EVT:DISP ───▶  7SEG 镜像同步更新     │
│  LED变化 ──── *EVT:LED ────▶  LED 阵列逐位解码      │
│                                                     │
│  物理按键 ◀──── *SET:KEY ────  虚拟按键被点击        │
│  显示控制 ◀──── *SET:DISP ────  控制面板下发指令     │
│                                                     │
│  ★ 1Hz 全量心跳自动校准，丢失事件自动恢复，无需重传   │
└─────────────────────────────────────────────────────┘
```
