...
MCU 端 \texttt{Process\_Command()} 收到一行指令后，首先调用 \texttt{Upper\_Copy()}（\texttt{main.c:2017-2026}）将整行处理为大写：
...
命令行通过 \texttt{strtok(work, "~\textbackslash t")} 分割参数（\texttt{main.c:2099}），任意数量的空格或 Tab 均可正确分割：
...
\texttt{Token\_Matches()}（\texttt{main.c:2028-2039}）是实现缩写识别的关键函数：
...
发送 \texttt{*SET:FORMAT RIGHT} 后，MCU 将 \texttt{g\_format} 切换为 \texttt{FMT\_RIGHT}（\texttt{main.c:2224}）。此后所有 \texttt{*GET} 应答和 \texttt{*EVT:DISP} 上报均触发逆序逻辑。

\texttt{Send\_OK\_Value()}（\texttt{main.c:2069-2082}）：
...
\texttt{SysTick\_Handler()} 每 1 毫秒触发一次，驱动四个分频标志（\texttt{main.c:271-278}）：
...
\texttt{TIMER0A\_Handler()} 每 1 毫秒执行一次（\texttt{main.c:288-311}），是系统最关键的中断服务例程：
...
UART 接收采用最高优先级中断（\texttt{main.c:316-345}），接收中断优先级最高，确保在 I\textsuperscript{2}C 事务进行时到达的字节不会因 FIFO 溢出而丢失。ISR 仅操作 ring buffer，绝不访问 I\textsuperscript{2}C，因此抢占显示扫描 ISR 是安全的。

\texttt{UART\_RxIsrHandler()}（\texttt{main.c:350-389}）的逐行组装逻辑：
...
\texttt{Display\_Refresh()}（\texttt{main.c:888-937}）每秒被 Timer0A ISR 调用 1000 次，每次刷新一位数码管（共 8 位），形成稳定的 125 Hz 刷新率（1000 / 8）。
...
主循环每秒调用一次 \texttt{Build\_Display()}（\texttt{main.c:1022}），按优先级组装 \texttt{disp\_buf[8]}：
...
\texttt{SegCode()}（\texttt{main.c:848-884}）将 ASCII 字符映射为 8 位段码（bit 0--6 对应 a--g，bit 7 为小数点），覆盖数字 0--9、大写字母 A--Z、横线 \texttt{-}和下划线 \texttt{\_}。段码表与 PC 端 \texttt{SevenSegmentDigit.MAP} 保持一致，确保数字孪生镜像的字符渲染完全一致。

\subsection{MCU 端按键状态机与事件分发}

\subsubsection{按键扫描（Key\_Scan）}

\texttt{Key\_Scan()}（\texttt{main.c:1247}）在 \texttt{flag\_10ms} 触发时由主循环调用，完成：
...
\texttt{Dispatch\_Key()}（\texttt{main.c:1311}）将事件类型分发到具体行为函数 \texttt{Handle\_Key()}或直接处理长按/连发：
...
传统闹钟基于绝对时间触发(如每天 07:00:00 响铃), 适用于周期性唤醒场景。但在课堂演示、专注力训练、短时提醒等场景中, 用户更需要基于\textbf{相对时长}的倒计时功能。该功能与基础闹钟形成互补: 闹钟按绝对时间、倒计时按相对时间, 二者独立并行。在此基础上增加了\textbf{板上按键可独立编辑与运行}的能力, 以及到点后\textbf{多情景收尾}: 滚动文本、闪烁庆祝、静默。
...
\textbf{MCU 全局状态与状态枚举 (main.c 行 139--150):}
...
\textbf{MCU 显示集成 (main.c 行 1031--1077):}
...
\textbf{MCU 1 秒递减 (main.c 行 1587--1605):}

\texttt{Time\_Tick\_1s()} 中 RUN 态下 \texttt{g\_cd\_remain\_s--}, 归零后转入 DONE 态并上报 \texttt{*EVT:CD DONE}, 根据情景决定是否激活蜂鸣器。(main.c 行 1588--1605)

\textbf{MCU 协议命令族 (main.c 行 1846--1895):}
...
\textbf{MCU LED 进度条 (main.c 行 1652--1662):}
...
\textbf{PC 端进度环 \texttt{CountdownRing} (twin\_panel.py 行 249--310):}
...
\textbf{PC 端 TTS 语音 (main.py 行 61--165):}

\texttt{TtsSpeaker} 类封装 \texttt{pyttsx3} / \texttt{win32com SAPI} 双通道。倒计时最后 5 秒(RUN 态 remain 1--5)时通过 \texttt{cd\_voice\_check} 复选框可选播报剩余秒数; DONE 时播报到点文本。

\textbf{PC 端 UI 控件 (main.py 行 428--483):}
...
\begin{enumerate}
    \item 点击 PC 端「时长」设置为 10 秒(或直接在板上按 EXT 进入编辑, 用 ADD/SAVE 设时);
    ...
    \item 倒计时期间 RUN/PAUSE 可按 EXT 暂停/继续, 长按 EXT 停止; 闹钟功能独立可用互不干扰。
\end{enumerate}

\textbf{关键代码位置}: \texttt{Countdown\_HandleKey (main.c 行 1792)} / \texttt{Build\_Display} 内联倒计时显示 (\texttt{main.c 行 1031}) / \texttt{Update\_Status\_LED} 内联倒计时 LED (\texttt{main.c 行 1652}) / \texttt{Time\_Tick\_1s} 内联递减 (\texttt{main.c 行 1587}) / \texttt{Countdown\_HandleCommand (main.c 行 1846)} / \texttt{Process\_Command} 派发 (\texttt{main.c 行 2343})。
