...
├── twin_panel.py                   # C3: 数字孪生镜像面板（自绘核心）
├── ntp_helper.py                   # E1: NTP 对时（3 服务器容错）
...
| 标注 | 下方显示 `L1\nHB` 格式（编号 + 英文含义） |
| 8 位映射 | `HB` / `ALM` / `EDIT` / `RXTX` / `SUN` / `RAIN` / `HOT` / `NTP` |
...
- LED6 (SUN) 亮 = 晴；LED7 (RAIN) 慢闪 = 雨/雪；LED8 (HOT) 亮 = 温度 ≥ 30°C
...
- 视频中 **PC 镜像面板 + MCU 实物** 必须同框可见（或镜头流畅切换）
- 两者 7SEG 显示内容、LED 亮灭状态需肉眼一致

### 7.2 必须演示的三种双向联动

| # | 操作 | 预期画面 |
|:--:|------|----------|
| 1 | **PC 点击 FUNC** | MCU 实物即刻进入编辑模式（数码管字段闪烁） |
| 2 | **MCU 按下 USER2** | PC 镜像面板 USER2 按钮同步高亮 200ms（蓝色脉冲） |
| 3 | **MCU 闹钟到点响铃** | PC 端弹窗提示"闹钟" + 日志区记录 `*EVT:ALARM` |

### 7.3 扩展功能演示

- **NTP 对时**：点击按钮 → 时间瞬间校准 → LED8 亮起
- **天气获取**：启动后 1 秒天气卡片自动更新 → 按 MCU USER2 短显天气
- **昼夜模式**：手动切换 DAY/NIGHT → 孪生面板跟随变暗/恢复
- **数据看板**：切换至"数据"Tab → 三张图表正常渲染
